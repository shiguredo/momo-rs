...
- Branch: feature/add-jetson-platform
- Polished: {YYYY-MM-DD}

## 目的

NVIDIA Jetson プラットフォーム (aarch64) のクロスコンパイル・パッケージング・実行対応。HW エンコーダ実装 (issue 0002) とは別の、プラットフォーム全体のサポートに関する課題。
...
- Jetson HW エンコーダ実装 (issue 0002) との統合

## pending 理由

クロスコンパイル環境の構築方針 (どのツールを使うか、L4T sysroot の取得方法、CI での再現性) について設計判断が必要。HW エンコーダ実装 (issue 0002) と密結合する部分もあり、両者の進め方を併せて検討する必要がある。

## 関連

- issue 0002 (pending) H.264 HW エンコード (Jetson) の実装: Jetson HW エンコーダ単体の課題
- issue 0011 (open) Windows プラットフォーム対応: 同種のプラットフォーム拡張
