...
pub(crate) fn start_fake_video_thread(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // --framerate は起動時検証で 1 以上が保証されている。0 が届いた場合は実装バグとして落とす
            let fps = std::num::NonZeroU32::new(fps).expect("BUG: fake video thread got fps=0");
            let frame_duration = Duration::from_nanos(1_000_000_000 / u64::from(fps.get()));
