...
use shiguredo_webrtc::{
    ...
    SetRemoteDescriptionObserver, SetRemoteDescriptionObserverHandler, StringVector, Thread,
    TimestampAligner, VideoDecoderFactory, VideoEncoderFactory, VideoTrackSource,
};
...
impl WebRtcEngine {
    pub(super) fn new(config: &P2PConfig) -> Result<Self, BoxError> {
        ...
        if !config.codec_selection.is_empty() && !use_codec_selection {
            warn!(target: "dev", "codec encoder/decoder options are ignored because --openh264 or --use-v4l2-encoder is set");
        }
        if use_codec_selection {
            info!(target: "dev", "using selected video codec factories");
            ...
            info!(target: "dev", "using OpenH264 encoder/decoder factory");
