...
use super::{AyameConfig, Direction, Watchdog};
...
impl AyameEngine {
    pub(super) fn new(config: &AyameConfig) -> Result<Self, BoxError> {
        ...
        // 映像コーデックファクトリの選択
        //
        // --openh264 / --use-v4l2-encoder は専用ファクトリを優先し、
        // それ以外で --{codec}-encoder / --{codec}-decoder が指定されていれば
        // Sora と同じ capability ベースのファクトリを使う。
        let use_codec_selection = !config.codec_selection.is_empty()
            && config.openh264_lib.is_none()
            && !config.use_v4l2_encoder;
        if !config.codec_selection.is_empty() && !use_codec_selection {
            tracing::warn!(target: "ayame", "codec encoder/decoder options are ignored because --openh264 or --use-v4l2-encoder is set");
        }
        if use_codec_selection {
            info!(target: "ayame", "using selected video codec factories");
            let (encoder_factory, decoder_factory) =
                crate::video_codec_factory::build_factories(&config.codec_selection)?;
            deps.set_video_encoder_factory(encoder_factory);
            deps.set_video_decoder_factory(decoder_factory);
        } else if config.use_v4l2_encoder {
            #[cfg(feature = "raspberrypi")]
            {
            ...
            info!(target: "ayame", "using OpenH264 encoder/decoder factory");
        ...
        let factory =
            shiguredo_webrtc::PeerConnectionFactory::create_modular(deps).map_err(wrtc_err)?;
...
pub(super) fn create_peer(
    ...
    struct PcObserver {
        cmd_tx: tokio::sync::mpsc::Sender<SignalingCommand>,
        /// 接続確立で監視期限を 60 秒へ切り替え、failed で接続を閉じる
        watchdog: Watchdog,
        #[cfg(target_os = "linux")]
    ...
    impl PeerConnectionObserverHandler for PcObserver {
        fn on_connection_change(&mut self, state: PeerConnectionState) {
            ...
            match state {
                // momo は ICE connected で retry_count_ を 0 にし期限を 60 秒へ切り替える。
                // sora_sdk と違い自前実装なので、PeerConnection の状態をそのまま使う
                PeerConnectionState::Connected => self.watchdog.on_connected(),
                // momo は ICE failed で WebSocket を閉じる。閉じた経路から再接続に入るので、
                // ここでは接続を閉じる命令だけ送る
                PeerConnectionState::Failed => {
                    // ICE コールバックは C++ スレッドから呼ばれるため blocking_send を使用
                    let _ = self.cmd_tx.blocking_send(SignalingCommand::Close);
                }
                _ => {}
            }
        }
    ...
    let pc = PeerConnection::create(&engine.factory, &rtc_config, deps).map_err(wrtc_err)?;
...
pub(super) fn add_transceivers(
    ...
    // 映像トランシーバー
    // degradation preference は RtpSender::set_parameters でしか設定できないが、
    // add_transceiver_with_track からは RtpSender を取得できない。
    // そのため、まず track なしのトランシーバーを作ってから add_track を呼ぶ。
    // libwebrtc は media type が一致し sender track が空のトランシーバーを再利用するため、
    // コーデックプリファレンス (transceiver) と degradation preference (sender) を両方設定できる。
    let mut video_transceiver = if sends_video(direction) {
    ...
    // コーデックプリファレンスを設定
    if video_codec_type.is_some() || audio_codec_type.is_some() {
