...
async fn main() -> noargs::Result<()> {
    ...
    let audio_input_device: Option<String> = noargs::opt("audio-input-device")
        ...
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    let audio_output_device: Option<String> = noargs::opt("audio-output-device")
        .ty("DEVICE")
    ...
    let framerate: String = noargs::opt("framerate")
        .ty("FRAMERATE")
        .doc("Video framerate (1-120)")
        .default("30")
        .take(&mut args)
        .then(|o| o.value().parse())?;
    let framerate: u32 =
        parse_framerate(&framerate).map_err(|e| noargs::Error::other(&noargs::raw_args(), e))?;
    let degradation_preference: Option<shiguredo_webrtc::DegradationPreference> =
        noargs::opt("degradation-preference")
            .ty("PREFERENCE")
            .doc("Specifies the quality that is maintained against video degradation (balanced, maintain-framerate, maintain-resolution)")
            .take(&mut args)
            .present_and_then(|o| {
                match o.value() {
                    "balanced" => Ok(shiguredo_webrtc::DegradationPreference::Balanced),
                    "maintain-framerate" => {
                        Ok(shiguredo_webrtc::DegradationPreference::MaintainFramerate)
                    }
                    "maintain-resolution" => {
                        Ok(shiguredo_webrtc::DegradationPreference::MaintainResolution)
                    }
                    other => Err(format!(
                        "invalid degradation-preference: '{other}' (balanced, maintain-framerate, maintain-resolution のいずれかを指定してください)"
                    )),
                }
            })?;
    ...
    let vp8_encoder: Option<String> = noargs::opt("vp8-encoder")
        ...
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    let vp8_decoder: Option<String> = noargs::opt("vp8-decoder")
        .ty("TYPE")
        .doc("VP8 Decoder")
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    ...
    let h264_encoder_value: Option<String> = noargs::opt("h264-encoder")
        ...
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    // momo 互換: --h264-encoder v4l2 は --use-v4l2-encoder と同じ扱いにする
    let use_v4l2_encoder = use_v4l2_encoder || h264_encoder_value.as_deref() == Some("v4l2");
    let h264_encoder_value = h264_encoder_value.filter(|value| value != "v4l2");
    ...
    // P2P / Ayame モード用のコーデック選択を組み立てる
    let codec_selection = video_codec_factory::CodecSelection {
        vp8_encoder: vp8_encoder.clone(),
        vp8_decoder: vp8_decoder.clone(),
        vp9_encoder: vp9_encoder.clone(),
        vp9_decoder: vp9_decoder.clone(),
        av1_encoder: av1_encoder.clone(),
        av1_decoder: av1_decoder.clone(),
        h264_encoder: h264_encoder_value.clone(),
        h264_decoder: h264_decoder_value.clone(),
        h265_encoder: h265_encoder_value.clone(),
        h265_decoder: h265_decoder_value.clone(),
    };
...
struct MomoConfig {
    ...
    vp8_encoder: Option<String>,
    vp8_decoder: Option<String>,
    vp9_encoder: Option<String>,
    vp9_decoder: Option<String>,
    av1_encoder: Option<String>,
    av1_decoder: Option<String>,
    /// P2P / Ayame モード用のコーデックエンコーダー/デコーダー選択
    codec_selection: video_codec_factory::CodecSelection,
    ...
    degradation_preference: Option<shiguredo_webrtc::DegradationPreference>,
...
/// `--framerate` の下限値 (momo と同じ)
const MIN_FRAMERATE: u32 = 1;
/// `--framerate` の上限値 (momo と同じ)
const MAX_FRAMERATE: u32 = 120;

/// `--framerate` の値を範囲検証付きでパースする
///
/// 0 はフェイク映像スレッドのゼロ除算に、上限超は `frame_duration` が 0 ナノ秒に丸まって
/// ビジーループになる。デバイスやスレッドを起動する前に失敗させる
fn parse_framerate(value: &str) -> Result<u32, String> {
    let parsed: u32 = value.parse().map_err(|_| {
        format!(
            "invalid framerate: '{value}' ({MIN_FRAMERATE}-{MAX_FRAMERATE} の整数を指定してください)"
        )
    })?;
    if !(MIN_FRAMERATE..=MAX_FRAMERATE).contains(&parsed) {
        return Err(format!(
            "invalid framerate: {parsed} ({MIN_FRAMERATE}-{MAX_FRAMERATE} の範囲を指定してください)"
        ));
    }
    Ok(parsed)
}
...
async fn run_p2p(
    ...
    let config = p2p::P2PConfig {
        ...
        degradation_preference: momo_config
            .degradation_preference
            .unwrap_or(shiguredo_webrtc::DegradationPreference::MaintainFramerate),
        video_codec_type,
        audio_codec_type,
        #[cfg(target_os = "linux")]
...
fn print_devices_json() -> Result<(), crate::error::BoxError> {
    ...
    let audio_input_list = AudioDeviceList::enumerate_input()
        .map_err(|e| format!("音声入力デバイスの列挙に失敗: {e}"))?;
    let audio_output_list = AudioDeviceList::enumerate_output()
        .map_err(|e| format!("音声出力デバイスの列挙に失敗: {e}"))?;
    ...
    let audio_inputs: Vec<ADevice> = audio_input_list
        .as_slice()
        .iter()
        .map(|d| ADevice {
            name: d.name().unwrap_or_default(),
        ...
        .collect();
    let audio_outputs: Vec<ADevice> = audio_output_list
        .as_slice()
        .iter()
        .map(|d| ADevice {
            name: d.name().unwrap_or_default(),
...
/// カンマ区切りの値を項目一覧に分割する
///
/// `--signaling-urls` と `--data-channel-label` で共用する。空要素と周囲の空白は除く。
...
async fn run_sora(
    ...
    let client_id: Option<String> = noargs::opt("client-id")
        .ty("ID")
        .doc("Client ID sent to Sora (1-255 bytes)")
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    let bundle_id: Option<String> = noargs::opt("bundle-id")
        .ty("ID")
        .doc("Bundle ID sent to Sora (1-255 bytes)")
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    let signaling_notify_metadata: Option<String> = noargs::opt("signaling-notify-metadata")
        .ty("JSON")
        .doc("Metadata included in signaling notify messages")
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    let data_channel_labels: Option<String> = noargs::opt("data-channel-label")
        .ty("LABELS")
        .doc("Additional DataChannels created at connect (comma separated, each label starts with #)")
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    let forwarding_filter: Option<String> = noargs::opt("forwarding-filter")
        .ty("JSON")
        .doc("Forwarding filters as a JSON array")
        .take(&mut args)
        .present_and_then(|o| o.value().parse())?;
    ...
    // sora_sdk には degradation preference を設定する API が存在しないため、明示指定はエラーにする
    if momo_config.degradation_preference.is_some() {
        return Err(noargs::Error::other(
            &noargs::raw_args(),
            "--degradation-preference は sora モードでは指定できません (sora_sdk に degradation preference API がありません)",
        ));
    }
    ...
    let signaling_urls = split_comma_values(&signaling_urls);
    // data-channel-label をカンマ区切りで分割
    let data_channel_labels = data_channel_labels
        .as_deref()
        .map(split_comma_values)
        .unwrap_or_default();

    let config = sora::SoraConfig {
        signaling_urls,
        channel_id,
        role,
        video,
        ...
        data_channel_labels,
        forwarding_filters: forwarding_filter,
        no_audio_device: momo_config.no_audio_device,
        no_video_input_device: momo_config.no_video_input_device,
        fake_capture_device: momo_config.fake_capture_device,
        use_v4l2_encoder: momo_config.use_v4l2_encoder,
    ...
    // プレイヤー (受信映像表示) 有効時: Sora 接続を別タスクで起動し、メインスレッドで SDL3 ループを回す
    // 受信映像 (recvonly / sendrecv のリモートトラック) をグリッド表示する。
    #[cfg(feature = "player")]
    if momo_config.use_player {
        let (display_cmd_tx, display_cmd_rx) =
            std::sync::mpsc::channel::<crate::display::DisplayCommand>();
        // 表示チャネルは接続を起動する前にだけ設定する (player feature 専用の変更)
        let mut config = config;
        config.display_cmd_tx = Some(display_cmd_tx);
        ...
        // 表示ウィンドウを閉じたら再接続を打ち切るための合図
        let stop = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));

        // VideoCapture は Windows の Media Foundation バックエンドが COM 初期化を
        // 保持するため Send を実装しない。Sora 接続を tokio::spawn に乗せられないので、
        // 専用スレッド上のカレントスレッドランタイムで実行する。
        let (result_tx, result_rx) =
            std::sync::mpsc::channel::<Result<(), crate::error::BoxError>>();
        let sora_stop = std::sync::Arc::clone(&stop);
        std::thread::Builder::new()
            ...
            .spawn(move || {
                let result = match tokio::runtime::Builder::new_current_thread()
                    ...
                    .build()
                {
                    Ok(runtime) => runtime.block_on(sora::run(config, metrics_state, sora_stop)),
                    Err(e) => Err(format!("Sora 接続用ランタイムの生成に失敗: {e}").into()),
                };
                // Sora 接続が終了したら表示ループに通知する
        ...
        tokio::task::block_in_place(|| {
            if let Err(e) = crate::display::run_display_loop(
                window_width,
                window_height,
                display_cmd_rx,
                shutdown_rx,
            ) {
                tracing::error!(target: "display", error = %e, "display loop error");
            }
        });

        // ウィンドウが閉じたら再接続を止める。現在の接続が終わるのを待って抜ける
    ...
    // 起動時に停止要求を受け付ける経路はないので、終了は SIGINT (Ctrl+C) になる
    let stop = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
    sora::run(config, metrics_state, stop)
        .await
        .map_err(|e| noargs::Error::other(&noargs::raw_args(), e))
}
#[cfg(test)]
mod tests {
    use super::{MAX_FRAMERATE, MIN_FRAMERATE, parse_framerate};

    /// momo と同じ範囲 (1〜120) が受理されること
    #[test]
    fn parse_framerate_accepts_momo_range() {
        for fps in [MIN_FRAMERATE, 24, 30, 60, MAX_FRAMERATE] {
            let parsed = parse_framerate(&fps.to_string());
            assert_eq!(parsed.ok(), Some(fps), "{fps} は受理するはず");
        }
    }

    /// 範囲外と非数値は起動時に拒否されること
    #[test]
    fn parse_framerate_rejects_invalid_values() {
        for value in ["0", "121", "1000", "-1", "abc", "", " 30"] {
            let parsed = parse_framerate(value);
            assert!(parsed.is_err(), "{value:?} は拒否するはず");
        }
    }
}
