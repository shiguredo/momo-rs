...
# momo のログには ANSI 制御文字が含まれるため、比較前に除く
ANSI_ESCAPE = re.compile(r"\x1b\[[0-9;]*m")


def read_log(path: str | Path) -> str:
    """log_path で指定したファイルへ書き出された momo のログを読み出す

    ANSI 制御文字を除くので、`retry_seconds=10` のように字段をそのまま照合できる。
    """
    return ANSI_ESCAPE.sub("", Path(path).read_text(encoding="utf-8"))
...
class Momo:
    ...
    def __init__(
        ...
        degradation_preference: Literal["balanced", "maintain-framerate", "maintain-resolution"]
        | None = None,
        use_sdl: bool = False,
        ...
        # 起動待ちの上限秒数。接続に失敗し続けるケースのテストで短くできる
        startup_timeout: int = 30,
        # momo のログを書き出すファイルパス。指定時は stdout / stderr をそのファイルへ送る
    ...
    def __enter__(self) -> Self:
        ...
        try:
            ...
            project_root = Path(__file__).parent.parent
            # tracing のログは stdout、sora_sdk 内部の出力は stderr に出る。
            # 既定では stdout を捨て stderr をパイプで受けるが、ログを精査するテストは
            # 両方を同じファイルへ逃がす (パイプを読まずに放置するとバッファで詰まる)。
            log_file = None
            if self.log_path:
                log_file = open(self.log_path, "w", encoding="utf-8")
                stdout_target: Any = log_file
                stderr_target: Any = log_file
            else:
            ...
            # 子プロセスが fd を継承したので親側のハンドルは閉じる
            if log_file:
                log_file.close()
            print(f"Started momo process with PID: {self.process.pid}")

            self._wait_for_startup(
                self.metrics_port, timeout=self.startup_timeout, initial_wait=self.initial_wait
            )
    ...
    def _build_args(self, mode: MomoMode, **kwargs: Any) -> list[str]:
        ...
        if kwargs.get("libcamera_control"):
            for key, value in kwargs["libcamera_control"]:
                args.extend(["--libcamera-control", f"{key}={value}"])
                ...
                if response.status_code == 200:
                    ...
                    for expected in default_conditions:
                        ...
                        for stat in stats:
                            # 種別と期待値の両方が一致する統計を探す
                            if (
                                stat.get("type") == expected_type
                                and stat.get(expected_key) == expected_value
                            ):
                                found = True
                                break
